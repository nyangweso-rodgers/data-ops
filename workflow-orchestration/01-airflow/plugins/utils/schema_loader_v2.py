...
class SchemaLoader:
    ...
    def _validate_schema(cls, schema: Dict[str, Any], table_name: str, target_type: str) -> None:
        ...
        # --- 3. Validate incremental columns ---
        active_incremental_cols = [
            col for col, config in source_columns.items()
            if config.get('incremental')
            and config.get('active', True)
        ]
        if len(active_incremental_cols) > 1:
            error_msg = "Multiple active incremental columns found: {active_incremental_cols}. Only one allowed."
            logger.error(error_msg)
            raise ValueError(error_msg)

        # --- 4. Log unused active columns ---
        unused_active_cols = [
            col for col, config in source_columns.items()
