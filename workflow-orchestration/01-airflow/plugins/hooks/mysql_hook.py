...
class MySqlHook:
    ...
    def execute_query(
        ...
        """
        Execute a query and return results, optionally in batches, respecting active columns in schema.
        ...
        Args:
            ...
            schema: Schema dictionary from SchemaLoader (optional, to filter active columns)
        ...
        try:
            with self.get_cursor() as cur:
                # If schema provided, filter columns by active: true
                if schema and 'source' in schema and 'columns' in schema['source']:
                    active_columns = [
                        col for col, config in schema['source']['columns'].items()
                        if config.get('active', True)
                    ]
                    if "SELECT" in query.upper() and "FROM" in query.upper():
                        # Replace SELECT * or broad column lists with active columns
                        if 'SELECT *' in query.upper():
                            columns_str = ', '.join([f"`{col}`" for col in active_columns])
                            query = query.replace('SELECT *', f'SELECT {columns_str}')
                            self.logger.debug(f"Modified query to select active columns: {query}")
