# dagster_pipeline/utils/postgres_utils/v1/postgres_utils.py
...
class PostgresUtils:
    ...
    def _ensure_state_dir():
        ...
        PostgresUtils.STATE_DIR.mkdir(parents=True, exist_ok=True)
    
    @staticmethod
    def _get_state_file(database: str, table: str) -> Path:
        """Get the state file path for a table"""
        PostgresUtils._ensure_state_dir()
        state_file = f"postgres_{database}_{table}.json"
        return PostgresUtils.STATE_DIR / state_file
    ...
    def load_incremental_state(
        ...
        state_file = PostgresUtils._get_state_file(database, table)
    ...
    def build_incremental_config(
        ...
        previous_state = PostgresUtils.load_incremental_state(context, database, table)
    ...
    def validate_source_db_table(
        ...
        context.log.info(f"Validating PostgreSQL source: {database}.{table}")
        
        with resource.get_connection(database) as conn:
            cursor = conn.cursor()
            ...
            if result[0] == 0:
                error_msg = f"Table {database}.{table} does not exist in PostgreSQL"
                context.log.error(error_msg)
                cursor.close()
                raise ValueError(error_msg)
            
            context.log.info(f"✓ Table {database}.{table} exists in PostgreSQL")
            cursor.close()
            return True
    ...
    def validate_columns(
        ...
        """Validate if required columns exist in the PostgreSQL table"""
    ...
    def stream_data(
        ...
        """Stream data from PostgreSQL as batches"""
        context.log.info(f"Streaming data from {database}.{table}")
        
        columns_str = ', '.join(f'"{col}"' for col in columns)
        query = f'SELECT {columns_str} FROM "public"."{table}"'
