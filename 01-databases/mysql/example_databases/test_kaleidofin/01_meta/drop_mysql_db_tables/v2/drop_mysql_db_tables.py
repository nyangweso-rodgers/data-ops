...
def drop_mysql_tables(connection: pymysql.connections.Connection, tables: List[str]):
...
def main():
    ...
    try:
        ...
        if not confirm_table_deletion(
            connection, tables_to_drop, config["database"], fast_mode=fast_mode
        ):
            logger.info("❌ Operation cancelled by user")
            sys.exit(0)
        ...
        # Drop tables
        print("\n🗑️  Dropping tables...\n")
        drop_mysql_tables(connection, tables_to_drop)
